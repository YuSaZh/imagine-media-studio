...
`latest` は最新の安定版を指します。バージョンを固定する場合は `:0.2.2` またはリリースページのイメージ digest を使用してください。バックアップ・移行・ロールバックは[デプロイガイド](./RELEASE.md)を参照してください。UI のデータベースバックアップには画像や動画は含まれません。
