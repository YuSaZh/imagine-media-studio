...
- Opening the editor expands the gallery thumbnail while fading out the main interface and fading in the editor. The decoded original replaces it without changing image bounds; failures retain the preview with retry, and Reduce motion skips expansion.
...
- Preferences can group editing series into one gallery entry with a count badge and a recently viewed, newest, or original cover; active editor generation shows only status and elapsed time. The nested concurrent-image option groups images from one recorded submission, including retries and reloads. Concurrent jobs appear as a series from submission through generation; open it before completion and view each output as it arrives. The nested uploaded-reference option controls whether uploaded references join results, and defaults off.
- Homepage data and visible images load first; editing modules then preload one at a time during idle periods. Replayed events coalesce refreshes to avoid duplicate request queues. Desktop work details use a wider floating card without resizing images. Details keep a fixed header, hide scrollbars, show generation duration, and show as many prompt lines as fit within the maximum card height, with expansion keeping the card size unchanged. Mobile cards cap at roughly 70% of screen height and dismiss on outside taps. Session validation no longer shows a separate transition page.
- The editor Composer animates between its single-line capsule and full input. The mobile main Composer also collapses when empty and unfocused, while text, references and settings interaction keep it expanded. Media and video input modes use sliding selection highlights; Reduce motion disables these transitions.
- Selection mode preserves the configured series grouping. Bulk favorite, project membership and deletion apply to every existing member of a selected series; counts and deletion confirmation show the actual number of works.
- The editor supports continuous desktop browsing and mobile horizontal swipes within a series/vertical swipes between entries; each view saves immediately and updates its cover, with older responses prevented from replacing a newer selection during rapid navigation. Single works omit the series strip.
...
Run affected local checks and reuse passing results for unchanged inputs; a release no longer requires duplicate full local CI. Push the final commit to `main` for one complete GitHub CI run, then push its matching `vX.Y.Z` tag. Release reuses successful CI for that exact commit, or waits for the existing run if tagged early, without rerunning quality, browser or source Docker tests. It builds one AMD64/ARM64 candidate, validates its exact digest, then publishes version tags, `latest` and the GitHub Release. See the [release procedure](./RELEASE.md#stable-releases).
