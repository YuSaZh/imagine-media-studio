...
- 瀑布流先显示占位卡片，再按真实尺寸逐张显示缩略图；优先加载最近且当前可见的作品，视口上下 1000px 内的缩略图以低优先级预加载，更远的图片暂不请求；下一批作品信息也提前 1000px 获取，慢图或失败图片不阻塞其他作品。
- 缩略图和视频封面使用按登录身份隔离的浏览器缓存，来回滚动直接复用；PWA 优先读取本地图片，最多保留 256 项、7 天。缓存过期后通过 ETag 校验，未变化的图片无需重复下载；退出登录和删除作品会清理相关缓存。
...
- 偏好设置支持浅色、深色和跟随系统的主题，以及简体中文、English、日本語三种界面语言；选择按账号保存，切换不清空草稿，原始图片与视频不做颜色反转。

- 图片与视频统一进入虚拟瀑布流，支持搜索、类型筛选、收藏和批量操作。
- 从图库进入编辑器时，缩略图放大的同时主界面渐隐、编辑界面渐显；原图解码完成后按相同尺寸替换，加载失败时保留预览并可重试，减少动效可跳过放大。
- 编辑器以系列展示原图与衍生图片、视频，生成中状态在主区域显示，缩略图位于输入框上方；失败任务支持复制提示词。
- 编辑器输入框支持单行胶囊与完整输入框的展开/收起动画。手机主输入框在空白且失焦时也会收起，有文字、参考素材或正在操作设置时保持展开。图片/视频及视频生成模式使用滑动选中背景；减少动效设置可关闭这些过渡。
- 选择模式保留偏好设置中的系列分组，批量收藏、加入项目和删除作用于所选系列的全部现有作品，数量与删除确认均按实际作品数显示。
- 偏好支持按系列合并作品并显示数量角标，封面可选最近查看、最新生成或最初原图；子选项“并发生图作为系列显示”可合并同一次提交的多张图片（需有批次记录，支持重试与刷新恢复）。编辑器生成中仅显示状态与计时。并发任务从提交及生成中即合并为系列，未完成时也可点击进入，逐张查看已完成结果；“上传参考图加入系列”子选项可控制参考图是否与结果合并，默认关闭。
...
- 编辑器支持桌面跨作品连续浏览、手机横滑系列内/竖滑入口间切换；最近查看逐次保存并即时更新封面，快速切换时旧响应不会覆盖新选择，单张作品不显示系列缩略条。
- 手机编辑器采用悬浮返回键与工具条、更小的系列缩略图；左右及上下浏览时当前作品和相邻作品同步跟手滑动，未聚焦时隐藏缺少模型提示。
- 手机端输入框底部不再显示模型、尺寸和张数摘要；模型选择保留在生成设置中。
...
- 工作区弹层、菜单和操作反馈具有轻量动效；「设置 → 偏好 → 减少动效」开启时关闭动画，也可跟随系统设置。
...
日常修改只做受影响范围的本地检查，已通过且输入未变的结果直接复用；发版不再要求重复跑完整本地 CI。最终提交推送到 `main` 后，由 GitHub 完成一次完整 CI，再推送匹配的 `vX.Y.Z` 标签。标签工作流复用同一提交的成功 CI；提前打标签时会等待现有运行，不重新执行质量、浏览器或源码 Docker 测试。随后构建一个 AMD64/ARM64 候选镜像，对其准确摘要做容器验收，再发布版本标签、`latest` 和 GitHub Release。详见[发版指南](./RELEASE.md#stable-releases)。
