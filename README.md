...
- 双端生成数量使用 1 / 2 / 4 / 8 和加号自定义（1–32）；数量表示独立单张任务，不受模型批量能力或参数规则限制，实际并发由服务器队列控制。
...
- 支持分辨率档位的通道直接传档位；仅支持像素尺寸的模型按画幅映射，例如 16:9 + 4K 为 3840x2160。Chat 通道兼容转换可映射的旧像素尺寸，实际输出尺寸由模型决定。
