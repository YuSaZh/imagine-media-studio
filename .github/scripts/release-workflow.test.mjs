...
assert.deepEqual(validateReleaseTag('v0.1.9', packageJson.version), { tag: 'v0.1.9', version: '0.1.9' });
...
assert.match(releaseNotes, /生成中即可打开系列/u);
assert.match(releaseNotes, /### 手机体验与缓存可靠性/u);
assert.match(releaseNotes, /### 贡献者/u);
assert.match(releaseNotes, /compare\/v0\.1\.8\.\.\.v0\.1\.9/u);
assert.match(releaseNotes, new RegExp(releaseDigest, 'u'));
assert.match(releaseNotes, /blob\/v0\.1\.9\/RELEASE\.md/u);
...
const testAttest = action(testImage.jobs.publish, attestRef);
assert.equal(testAttest.with['subject-name'], '${{ env.IMAGE }}');
assert.equal(testAttest.with['subject-digest'], '${{ steps.push.outputs.digest }}');
assert.equal(testAttest.with['push-to-registry'], false, 'Test attestations must not create a second tagged GHCR version.');
assert.equal(testAttest.with['create-storage-record'], false);
assert.equal(testImage.jobs.publish.permissions['artifact-metadata'], undefined);
