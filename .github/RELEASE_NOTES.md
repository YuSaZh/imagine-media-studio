...
Check all release-facing versions, render the generated notes locally with a fixture digest, and run `pnpm test:release` when versions, notes or release logic change. Complete `pnpm run ci` locally before publication. Reuse results only for unchanged inputs, scope and environment; notes-only follow-ups require release validation. Confirm all authored sections survive extraction, historical sections are excluded, links are correct, and prose has no manual wrapping. Push the final commit to main and let its complete CI pass once, then push the matching tag. An early tag waits for that exact commit's main CI instead of launching another suite. Failed, cancelled, skipped or missing required CI jobs block publication. Release then builds one multi-platform candidate, attests it, tests its exact digest, and promotes the same image before creating the GitHub Release. Let those gates complete before announcing the release, and verify the final published body and immutable digest afterward.
