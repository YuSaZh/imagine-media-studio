...
Check all release-facing versions, render the generated notes locally with a fixture digest, and run `pnpm test:release`. Confirm that all authored sections survive extraction, historical sections are excluded, every public link is correct, and prose has no manual wrapping. Require successful CI on the release commit before pushing the tag. Let the existing candidate, attestation, smoke, promotion, and GitHub Release workflow complete before announcing the release. Verify the final published body and immutable image digest afterward.
