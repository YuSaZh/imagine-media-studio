...
describe('public provider input delivery', () => {
  it('delivers only signed image inputs, preserves private API auth, and rejects expired or deleted images', async () => {
    ...
    const config = loadConfig({ NODE_ENV: 'test', DATA_DIR: root, PUBLIC_BASE_URL: 'https://studio.example', APP_PASSWORD: 'test-input-password' });
    ...
    try {
      ...
      const headers = { authorization: `Basic ${Buffer.from('test:test-input-password').toString('base64')}` };
