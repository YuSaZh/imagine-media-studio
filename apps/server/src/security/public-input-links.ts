...
export class PublicInputLinks {
  constructor(private readonly secret: string, private readonly baseUrl: string, private readonly now: () => number = Date.now, readonly ttlSeconds = 900) {}
  ...
  create(asset: AssetRecord): string {
    ...
    return `${this.baseUrl.replace(/\/$/, '')}/media-inputs/${encodeURIComponent(asset.id)}/${expires}/${this.signature(asset, expires)}`;
  }
