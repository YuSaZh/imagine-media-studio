...
describe('SQLite initialization', () => {
  it('upgrades an existing database with manual series links and reopens it', async () => {
    const fixture = await databaseFixture('imagine-manual-series-');
    const migration = '0012_manual_series.sql';
    ...
    expect(upgraded.sqlite.prepare('SELECT count(*) AS count FROM asset_series_links').get()).toEqual({ count: 0 });
  ...
  it('upgrades existing jobs without inventing batches and retains new batches after restart', async () => {
    const fixture = await databaseFixture('imagine-generation-batch-upgrade-');
    const migration = '0011_generation_batches.sql';
    await rm(resolve(fixture.migrations, '0012_manual_series.sql'));
    await rm(resolve(fixture.migrations, migration));
    const manifestPath = resolve(fixture.migrations, 'manifest.json');
    const manifest = JSON.parse(await readFile(manifestPath, 'utf8')) as { migrations: Record<string, string> };
    delete manifest.migrations[migration];
    ...
    await writeFile(manifestPath, JSON.stringify(manifest));
    const legacy = createDatabase(fixture.databasePath, fixture.migrations);
    const provider = new ProviderRepository(legacy.orm).create({ name: 'Batch fixture', type: 'mock' });
    const input = { providerId: provider.id, modelId: 'mock-image-v1', operation: 'image.generate' as const, prompt: 'Existing prompt', inputs: [] };
    const old = new JobRepository(legacy.orm).create(input);
    legacy.sqlite.close();
    await copyFile(resolve(migrationsDirectory, migration), resolve(fixture.migrations, migration));
    await addManifestEntry(fixture.migrations, migration);
    const upgraded = createDatabase(fixture.databasePath, fixture.migrations);
    expect(new JobRepository(upgraded.orm).get(old.id)?.request.prompt).toBe('Existing prompt');
    expect(upgraded.sqlite.prepare('SELECT * FROM job_generation_batches').all()).toEqual([]);
    const batch = new JobRepository(upgraded.orm).createBatch(input, 2);
    upgraded.sqlite.close();
    const reopened = createDatabase(fixture.databasePath, fixture.migrations);
    const records = reopened.sqlite.prepare('SELECT job_id, batch_id FROM job_generation_batches').all() as { job_id: string; batch_id: string }[];
    expect(records.map(row => row.job_id).sort()).toEqual(batch.map(job => job.id).sort());
    expect(new Set(records.map(row => row.batch_id)).size).toBe(1);
    expect(reopened.sqlite.prepare('PRAGMA foreign_key_check').all()).toEqual([]);
    reopened.sqlite.close();
  });
  ...
  it('adds public-by-default project privacy to existing databases and retains saved changes', async () => {
    ...
    await rm(resolve(fixture.migrations, migration));
    await rm(resolve(fixture.migrations, '0011_generation_batches.sql'));
    await rm(resolve(fixture.migrations, '0012_manual_series.sql'));
    const manifestPath = resolve(fixture.migrations, 'manifest.json');
    const manifest = JSON.parse(await readFile(manifestPath, 'utf8')) as { migrations: Record<string, string> };
    delete manifest.migrations[migration];
  ...
  it('applies the committed migrations exactly once and configures required pragmas', async () => {
    ...
    expect(
      ...
      { version: '0012_manual_series.sql' },
    ]);
  ...
  it('rejects a directory that skips 0006 while claiming a later migration', async () => {
    ...
    await writeFile(
      resolve(fixture.migrations, '0013_after_missing-0006.sql'),
  ...
  it('checks drift before applying a newer migration', async () => {
    ...
    await writeFile(
      resolve(fixture.migrations, '0013_after_drift.sql'),
      'CREATE TABLE must_not_be_applied (id INTEGER PRIMARY KEY);\n',
    );
    await addManifestEntry(fixture.migrations, '0013_after_drift.sql');
  ...
  it('rejects a checksum column whose lock state is missing before pending migrations run', async () => {
    ...
    first.sqlite.close();
    await writeFile(
      resolve(fixture.migrations, '0013_after_missing_lock.sql'),
      'CREATE TABLE must_not_be_applied_without_lock (id INTEGER PRIMARY KEY);\n',
    );
    await addManifestEntry(fixture.migrations, '0013_after_missing_lock.sql');
  ...
  it('rolls back a pending migration that removes the checksum column', async () => {
    ...
    first.sqlite.close();
    await writeFile(
      resolve(fixture.migrations, '0013_drop_checksum.sql'),
    ...
    await addManifestEntry(fixture.migrations, '0013_drop_checksum.sql');
    ...
    expect(
      sqlite.prepare('SELECT version FROM schema_migrations WHERE version = ?').get('0013_drop_checksum.sql'),
    ).toBeUndefined();
    expect(
  ...
  it('rolls back a pending migration that forges an applied history row', async () => {
    ...
    first.sqlite.close();
    await writeFile(
      resolve(fixture.migrations, '0013_forge_history.sql'),
    ...
    await addManifestEntry(fixture.migrations, '0013_forge_history.sql');
    ...
    expect(
      sqlite.prepare('SELECT version FROM schema_migrations WHERE version = ?').get('0013_forge_history.sql'),
    ).toBeUndefined();
    expect(
      sqlite.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'must_not_be_applied_after_forged_history'").get(),
    ).toBeUndefined();
    expect(sqlite.prepare('SELECT COUNT(*) AS count FROM schema_migrations').get()).toEqual({ count: 13 });
    sqlite.close();
  });
  ...
  it('rolls back a pending migration that recreates the lock table without triggers', async () => {
    ...
    first.sqlite.close();
    await writeFile(
      resolve(fixture.migrations, '0013_recreate_lock_without_triggers.sql'),
      `DROP TRIGGER schema_migrations_checksum_immutable;
       DROP TRIGGER schema_migrations_row_immutable;
    ...
    await addManifestEntry(fixture.migrations, '0013_recreate_lock_without_triggers.sql');
    ...
    expect(
      sqlite.prepare('SELECT version FROM schema_migrations WHERE version = ?').get('0013_recreate_lock_without_triggers.sql'),
    ).toBeUndefined();
    sqlite.close();
  });
