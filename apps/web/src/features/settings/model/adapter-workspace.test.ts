...
describe('workspace adapter presentation', () => {
  const render = (props: Parameters<typeof CustomAdapterWorkspace>[0]) => renderToStaticMarkup(
    createElement(Tooltip.Provider, null, createElement(CustomAdapterWorkspace, props)),
  );
  it('renders the new definition editor with accessible names', () => {
    const markup = render({ mode: 'custom-http' });
    expect(markup).toContain('aria-label="适配器定义"');
    expect(markup).toContain('保存定义');
    expect(markup).not.toContain('Adapter workspace');
  });
  it('retains local fields while offline and blocks remote writes', () => {
    const markup = render({ online: false });
    expect(markup).toContain('aria-label="适配器定义"');
    expect(markup).toContain('离线时无法修改适配器');
    expect(markup).toMatch(/<button class="primary-command" disabled=""/);
  });
  it('provides trusted source selection without embedding source code', () => {
    const markup = render({ mode: 'trusted-js', trustedJs: { manifest: trustedManifest } });
    expect(markup).toContain('aria-label="JavaScript 源文件"');
    expect(markup).toContain('安装并绑定');
    expect(markup).not.toContain('sourceFile');
  });
});
...
import { CustomAdapterWorkspace } from '../../workspace/adapter-workspace';
