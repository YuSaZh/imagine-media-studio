...
import type * as WorkspaceModule from '../../workspace/adapter-workspace.js';
import type { CustomAdapterWorkspaceActions, AdapterRevision } from '../model/adapter-workspace.js';
...
vi.mock('../../workspace/adapter-workspace.js', async () => {
  const actual = await vi.importActual<typeof WorkspaceModule>('../../workspace/adapter-workspace.js');
  return {
    ...actual,
...
describe('custom adapter workspace revision actions', () => {
  it('rebounds SSR-captured actions to the active Provider and omits the workspace while closed', async () => {
    const { CustomAdapterWorkspaceContainer } = await import('./adapter-controller.js');
  ...
  it('uses a fresh container identity and resets every session field on provider/open changes', async () => {
    ...
    } = await import('./adapter-controller.js');
  ...
  it('retains local test fields while a full immutable ref remounts the child workspace', async () => {
    const { customAdapterWorkspaceKey, mapCustomDefinitionToDraft } = await import('./adapter-controller.js');
