...
export function useWorkspaceJobs(status?: string, excludePrivate = false, series: { groupBySeries?: boolean; groupConcurrentImages?: boolean; groupUploadedReferences?: boolean } = {}) {
  return useInfiniteQuery({
    queryKey: [...keys.jobs, 'workspace', status ?? 'all', excludePrivate, series],
    queryFn: ({ pageParam }) => internalClient.listJobs({ ...series, limit: 60, ...(excludePrivate ? { excludePrivate: true } : {}), ...(pageParam ? { cursor: pageParam } : {}), ...(status ? { status } : {}) }),
    initialPageParam: undefined as string | undefined,
    getNextPageParam: page => page.nextCursor ?? undefined,
