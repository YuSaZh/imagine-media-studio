...
export function Workspace() {
  ...
  const mediaQuery = useMedia({ excludePrivate, publicProjectIds: projects.filter(project => !project.isPrivate).map(project => project.id).sort(), kind: filter, saved: section === 'saved', projectId, search: searchQuery });
  ...
  return <div className="imagine-app">
    ...
    {viewer && !viewerQuery.isError && <Suspense fallback={<Panel open title="作品编辑" onClose={closeViewer}><p className="loading-state">正在加载编辑工作区…</p></Panel>}><MediaEditingWorkspace models={models} projectId={projectId} layout={layout} drafts={editingDrafts.current} onSelectResult={item => openViewer(item.id)} item={viewer} index={Math.max(0, items.findIndex(item => item.id === viewer.id))} total={Math.max(1, items.length)} projects={projects} online={online} busy={busy > 0} providerName={catalog.providers.data?.find(provider => provider.id === viewer.providerId)?.name ?? '本地上传'} onClose={closeViewer} onMove={delta => { const index = items.findIndex(item => item.id === viewer.id); const next = items[(Math.max(0, index) + delta + items.length) % items.length]; if (next) openViewer(next.id); }} onSave={() => save(viewer)} onDelete={() => remove([viewer])} onProject={(id, included) => void run('project-membership', async () => { if (included) await internalClient.addCollectionAssets(id, [viewer.id]); else await internalClient.removeCollectionAsset(id, viewer.id); }, included ? '已加入项目' : '已移出项目')} onNotice={notify} /></Suspense>}
