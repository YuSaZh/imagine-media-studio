...
export function Workspace() {
  ...
  const remove = (targets: MediaItem[]) => setConfirmation({ title: t("删除 {0} 件作品？", [targets.length]), description: t("这会删除服务器上的作品及其媒体文件，此操作无法撤销。"), action: async () => {
  ...
  return <div className={`imagine-app ${layout === 'desktop' && railExpanded ? 'has-expanded-rail' : ''}`}>
    <aside className={`side-rail ${railExpanded ? 'is-expanded' : ''}`} onKeyDown={event => { if (event.key === 'Escape') setRailExpanded(false); }}>
      <button className="identity" aria-label={railExpanded ? t("收起侧边栏") : t("展开侧边栏")} aria-expanded={railExpanded} aria-controls="desktop-rail-navigation" onClick={() => setRailExpanded(current => !current)}><Sparkles size={25} strokeWidth={1.7} /></button>
      ...
      <div className="rail-bottom"><Tool label={t("连接与偏好")} onClick={() => go('/settings/providers')}><Settings2 size={21} />{railExpanded && <span className="rail-label">{t("连接与偏好")}</span>}</Tool><span className="avatar">I</span></div>
    </aside>
    <main ref={mainRef} className={`workspace workspace-${layout} ${hasGallery ? 'has-gallery' : ''} section-${isCreate ? 'create' : section}`}>
      <header className="workspace-header"><div className="workspace-location"><Tool label={t("打开导航")} className="mobile-menu" onClick={() => setMenuOpen(true)}><Menu size={21} /></Tool><button className="wordmark" onClick={() => go('/imagine')}>Imagine<span className="wordmark-dot">.</span></button><span className="header-divider" /><Options label={t("选择项目")} className="current-location" trigger={<><span className="project-trigger-text">{section === 'settings' ? t("设置") : section === 'jobs' ? t("生成任务") : title}</span><ArrowUpRight size={12} /></>}><Choice active={!projectId} onClick={() => go('/imagine')}>{t("最近创作")}</Choice>{projects.map(project => <Choice key={project.id} active={projectId === project.id} onClick={() => go(`/projects/${project.id}`)}><Folder size={15} />{project.name}</Choice>)}</Options></div><div className="header-actions"><button className="task-indicator" aria-label={t("生成任务")} onClick={() => setTaskOpen(true)}>{activeJobs.length ? <LoaderCircle className="spin" size={16} /> : <Clock3 size={17} />}<span>{t("任务")}</span>{activeJobs.length > 0 && <b>{activeJobs.length}</b>}</button><Tool className="mobile-settings" label={t("连接与偏好")} onClick={() => go('/settings/providers')}><Settings2 size={19} /></Tool></div></header>
