...
export function Workspace() {
  ...
  return <div className={`imagine-app ${layout === 'desktop' && railExpanded ? 'has-expanded-rail' : ''}`}>
    ...
    {selecting && <div className={`batch-toolbar ${isCreate ? '' : 'batch-toolbar-library'}`}><span>已选 {selected.length} 件</span><Tool label="收藏所选作品" disabled={!selected.length || !online || busy > 0} onClick={() => void run('bulk-save', async () => { for (const item of selected) await internalClient.patchAsset(item.id, true); }, '已加入收藏')}><Bookmark size={18} /></Tool><Options label="将所选作品加入项目" disabled={!selected.length || !online || busy > 0} trigger={<FolderPlus size={18} />}><div className="option-heading">项目</div>{projects.map(project => <Choice key={project.id} active={false} onClick={() => void run('bulk-project', async () => { if (!selected.length) return; await internalClient.addCollectionAssets(project.id, selected.map(item => item.id)); setSelected([]); setSelecting(false); }, '已加入项目')}>{project.name}</Choice>)}</Options><Tool label="删除所选作品" disabled={!selected.length || !online || busy > 0} onClick={() => remove(selected)}><Trash2 size={18} /></Tool><Tool label="关闭多选" onClick={() => { setSelected([]); setSelecting(false); }}><X size={18} /></Tool></div>}
    ...
    {viewer && !viewerQuery.isError && <Suspense fallback={viewerEntry.activeId === viewerId ? <span className="viewer-entry-status" role="status">正在打开作品…</span> : <Panel open title="作品编辑" onClose={closeViewer}><p className="loading-state">正在加载编辑工作区…</p></Panel>}><MediaEditingWorkspace entryMotion={viewerEntry.motion} entryOpening={viewerEntry.activeId === viewerId} models={models} projectId={projectId} layout={layout} drafts={editingDrafts.current} onSelectResult={selectViewerItem} onBrowseEntry={moveViewerEntry} onResolveEntry={resolveViewerEntry} item={viewer} index={Math.max(0, items.findIndex(item => item.id === viewer.id))} total={Math.max(1, items.length)} projects={projects} online={online} busy={busy > 0} providerName={catalog.providers.data?.find(provider => provider.id === viewer.providerId)?.name ?? '本地上传'} onClose={closeViewer} onMove={delta => { const index = items.findIndex(item => item.id === viewer.id); const next = items[(Math.max(0, index) + delta + items.length) % items.length]; if (next) openViewer(next.id); }} onSave={() => save(viewer)} onDelete={() => remove([viewer])} onProject={(id, included) => void run('project-membership', async () => { if (included) await internalClient.addCollectionAssets(id, [viewer.id]); else await internalClient.removeCollectionAsset(id, viewer.id); }, included ? '已加入项目' : '已移出项目')} onNotice={notify} /></Suspense>}
    ...
    <Panel title="生成任务" open={taskOpen} onClose={() => setTaskOpen(false)} className="task-panel">{renderJobs}</Panel>
