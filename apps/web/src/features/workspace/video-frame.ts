...
async function waitForFrame(video: HTMLVideoElement, signal?: AbortSignal): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const failed = () => { cleanup(); reject(new Error(t("视频画面无法读取，请等待视频加载后重试"))); };
...
export async function captureVideoFrame(video: HTMLVideoElement, signal?: AbortSignal): Promise<CapturedVideoFrame> {
  video.pause();
  if (video.error) throw new Error(t("视频无法播放，不能截取当前帧"));
  await waitForFrame(video, signal);
  signal?.throwIfAborted();
  const width = video.videoWidth, height = video.videoHeight, timeSeconds = video.currentTime;
  if (!Number.isSafeInteger(width) || !Number.isSafeInteger(height) || width < 1 || height < 1 || width * height > MAX_FRAME_PIXELS) throw new Error(t("视频画面超过截图尺寸限制"));
  if (!Number.isFinite(timeSeconds) || timeSeconds < 0) throw new Error(t("视频播放位置无效"));
  const canvas = video.ownerDocument.createElement('canvas'); canvas.width = width; canvas.height = height;
  try {
    const context = canvas.getContext('2d');
    if (!context) throw new Error(t("当前浏览器无法截取视频画面"));
    try { context.drawImage(video, 0, 0, width, height); } catch { throw new Error(t("视频画面不允许截图，请使用可正常播放的本地视频")); }
    const blob = await new Promise<Blob>((resolve, reject) => {
      const cleanup = () => { clearTimeout(timer); signal?.removeEventListener('abort', aborted); };
      const aborted = () => { cleanup(); reject(signal?.reason ?? new DOMException('Aborted', 'AbortError')); };
      const timer = setTimeout(() => { cleanup(); reject(new Error(t("视频截图超时，请重试"))); }, 8000);
      signal?.addEventListener('abort', aborted, { once: true });
      try { canvas.toBlob(value => { cleanup(); if (value) resolve(value); else reject(new Error(t("视频截图编码失败，请重试"))); }, 'image/png'); }
      catch { cleanup(); reject(new Error(t("视频画面不允许截图，请使用可正常播放的本地视频"))); }
    });
    signal?.throwIfAborted();
    if (blob.type !== 'image/png') throw new Error(t("视频截图编码格式不受支持"));
    if (blob.size < 1 || blob.size > MAX_FRAME_BYTES) throw new Error(t("视频截图超过图片大小限制"));
