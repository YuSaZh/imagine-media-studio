...
import { Tool } from './ui';
import type { ImageDimensionConstraints } from '@imagine/shared';

export function alignImageDimension(value: string | number, step = 16, max = 16384): number | undefined {
  const number = Number(value);
  if (!Number.isFinite(number) || number <= 0 || number > max || !Number.isInteger(step) || step < 1) return undefined;
  const aligned = Math.min(Math.max(step, Math.round(number / step) * step), Math.floor(max / step) * step);
  return aligned > 0 ? aligned : undefined;
}

export function linkedImageDimensions(width: string, height: string, edited: 'width' | 'height', ratio: string, locked: boolean, constraints: ImageDimensionConstraints = {}): { width: number; height: number } | undefined {
  let w = alignImageDimension(width, constraints.multipleOf, constraints.maxWidth);
  let h = alignImageDimension(height, constraints.multipleOf, constraints.maxHeight);
  const match = /^([1-9]\d*):([1-9]\d*)$/.exec(ratio);
  if (locked && match) {
    if (edited === 'width' && w !== undefined) h = alignImageDimension(w * Number(match[2]) / Number(match[1]), constraints.multipleOf, constraints.maxHeight);
    if (edited === 'height' && h !== undefined) w = alignImageDimension(h * Number(match[1]) / Number(match[2]), constraints.multipleOf, constraints.maxWidth);
  }
  if (w === undefined || h === undefined || w * h > 100_000_000) return undefined;
  return { width: w, height: h };
}

export function CustomDimensions({ value, ratio, accepts, onApply, onUnlock, unlockDisabled = false, constraints = {} }: {
  value: string; ratio: string; accepts: (value: string) => boolean; onApply: (value: string) => void; onUnlock: () => void; unlockDisabled?: boolean; constraints?: ImageDimensionConstraints | undefined;
}) {
  ...
  const normalize = (side: 'width' | 'height', lock = locked) => {
    const next = linkedImageDimensions(width, height, side, ratio, lock, constraints);
  ...
  const change = (side: 'width' | 'height', value: string) => {
    ...
    const next = linkedImageDimensions(side === 'width' ? value : width, side === 'height' ? value : height, side, ratio, locked, constraints);
