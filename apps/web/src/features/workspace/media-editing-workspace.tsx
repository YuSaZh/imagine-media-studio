...
function EditingSession(props: ViewerProps & { motion: ViewerMotion; models: WorkspaceModel[]; projectId: string | null; layout: WorkspaceLayout; drafts: MediaEditingDrafts; onSelectResult: (item: MediaItem) => void; onBrowseEntry: (delta: number, memberIds: string[], boundary?: boolean) => Promise<boolean>; onResolveEntry: (delta: number, memberIds: string[], boundary?: boolean) => Promise<MediaItem | null> }) {
  ...
  const captureFrame = async (forMask = false): Promise<AssetDto> => {
    ...
    if (!props.online || !video) throw new Error(t("请先加载并定位视频"));
    if (captureAbort.current) throw new Error(t("正在准备视频帧，请稍候"));
    video.pause();
    ...
    if (cached && (draftRef.current.mask || forMask && !video.seeking && Math.abs(cached.timeSeconds - video.currentTime) < 0.001)) {
      try { return (await internalClient.getAsset(cached.asset.id)).asset; } catch { if (draftRef.current.mask && !forMask) throw new Error(t("这张临时帧已过期，请重新打开蒙版后发送")); }
    }
  ...
  const openMask = async () => {
    ...
    catch (failure) { if (mounted.current) setError(failure instanceof Error ? failure.message : t("视频截图失败，请重试")); }
  };
  ...
  const submit = async (creation: Creation) => {
    if (activeJobId) { setError(t("请先选择系列中的图片或视频作为编辑原图")); return; }
    if (!original || sourceQuery.isError) { setError(t("原素材暂时无法读取，请重试")); return; }
    ...
    try {
      ...
      void refresh();
    } catch (failure) { if (mounted.current) setError(failure instanceof Error ? failure.message : t("生成提交失败")); }
    finally { lock.current = false; if (mounted.current) setSubmitting(false); }
  };
  const jobAction = async (id: string, retry: boolean) => {
    if (retry && sourceIsVideo && jobs.some(query => query.data?.job.id === id && query.data.job.operation.startsWith('image.'))) { setError(t("临时视频帧已清理，请确认视频位置后重新发送")); return; }
  ...
  const stageContent = activeJobId || submitting && !sourceIsVideo ? <div className={`editing-generation-stage pending-study ${activeJob && ['failed', 'rejected', 'expired', 'cancelled'].includes(activeJob.status) ? 'is-failed' : ''}`} role="status" aria-label={t("编辑生成状态")}>
  ...
  const controls = <>
    <div className={`image-editing-controls ${expanded ? 'is-expanded' : ''}`}>
      {(seriesItems.length > 1 || unresolvedJobs.length > 0 || submitting || series.isError) && <div className="editing-results" aria-label={t("编辑系列")}>{seriesItems.map((item, index) => <div className={`editing-result ${!activeJobId && item.id === props.item.id ? 'is-selected' : ''}`} key={item.id}><button type="button" aria-label={index === 0 ? t("查看原图") : item.kind === 'video' ? t("查看生成视频") : t("编辑此生成结果")} aria-pressed={!activeJobId && item.id === props.item.id} title={item.title} onClick={() => selectItem(item)}><img src={item.thumbnail} alt={index === 0 ? t("原图") : t("生成结果")} />{item.kind === 'video' && <Play className="series-video-mark" size={16} fill="currentColor" />}</button></div>)}
      {unresolvedJobs.map(job => <div className={`editing-result ${activeJobId === job.id ? 'is-selected' : ''}`} key={job.id}><button type="button" className="series-job" aria-label={t("查看任务 {0}", [job.prompt])} aria-pressed={activeJobId === job.id} onClick={() => selectJob(job.id)}><Sparkles size={18} /><GenerationStatus status={job.status} createdAt={job.createdAt} completedAt={job.completedAt} /></button></div>)}
      {series.isError && <button type="button" onClick={() => void series.refetch()}>{t("重试加载系列")}</button>}{series.data?.truncated && <span>{t("系列内容较多，当前仅显示部分作品")}</span>}
      </div>}
      {capturing && <p className="composer-notice editing-error" role="status">{t("正在截取并上传当前视频帧…")}</p>}
      {sourceQuery.isError && <p className="composer-notice editing-error" role="alert">{t("原素材暂时无法读取")}<button type="button" onClick={() => void sourceQuery.refetch()}>{t("重试")}</button></p>}
      {error && <p className="composer-notice editing-error" role="alert">{error}<button type="button" onClick={() => setError('')}>{t("关闭")}</button></p>}
      {draft.mode === 'image' && <Tool label={t("编辑蒙版")} className={`editing-mask-entry ${draft.mask ? 'is-active' : ''}`} aria-pressed={!!draft.mask} disabled={!props.online || !original || !candidates.length || submitting || capturing || !!activeJobId} onPointerDown={event => event.preventDefault()} onClick={() => void openMask()}><Brush size={18} /></Tool>}
      <Composer stateKey={`${draft.mode}:${videoMode}:${model?.key ?? ''}`} editing={{ compact: !expanded, onExpand: () => setExpanded(true), sourceId: source?.id ?? props.item.id, ...(sourceIsVideo ? { videoModes: continuationModes, busy: capturing, ...(draft.mode === 'image' && draft.mask && draft.frame ? { sourceLabel: t("参考帧 {0}", [videoFrameLabel(draft.frame.timeSeconds)]) } : {}) } : {}) }} operationOverride={operation} layout={props.layout} projectId={scope} prompt={draft.prompt} onPrompt={prompt => update({ prompt })} mode={draft.mode} onMode={mode => void changeMode(mode)} videoMode={videoMode} onVideoMode={mode => { if (mode === 'edit' || mode === 'extend') update({ videoMode: mode }); }} models={models} model={model} onModel={key => patchSettings.mutate(updateGenerationMemory(settings.data?.settings, scope, draft.mode, { selected: key }))} references={inputs} uploads={uploads} onFiles={(files, rejected) => uploads.addFiles(files, { existingCount: draft.references.length + 1, maxItems: maximum, ...(rejected ? { preliminaryRejections: rejected } : {}) })} onRemove={id => { if (id !== source?.id) update({ references: draft.references.filter(asset => asset.id !== id), ...(draft.mask?.id === id ? { mask: null } : {}) }); }} onCreate={creation => void submit(creation)} onLibrary={() => setPickerOpen(true)} onConnections={() => props.onNotice(t("请先在模型与服务中添加支持当前操作的模型"))} online={props.online} submitting={submitting} loading={capturing || !original || sourceQuery.isError || !!activeJobId} focusToken={0} />
    </div>
    ...
    {maskOpen && source?.type === 'image' && <Suspense fallback={<Panel open title={t("局部编辑")} onClose={() => setMaskOpen(false)}><LoaderCircle className="spin" /></Panel>}><Editor assetId={source!.id} {...(draft.document ? { initialDocument: draft.document } : {})} onClose={() => setMaskOpen(false)} onApply={(_source, mask, document) => { update({ mask, document }); setMaskOpen(false); void refresh(); }} /></Suspense>}
  </>;
