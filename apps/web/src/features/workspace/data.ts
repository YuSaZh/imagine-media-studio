...
export interface MediaFilter { groupConcurrentImages?: boolean; groupBySeries?: boolean; seriesCover?: 'recent' | 'latest' | 'original'; excludePrivate?: boolean; publicProjectIds?: readonly string[]; kind: 'all' | MediaKind; saved: boolean; projectId: string | null; search: string; }
...
export async function fetchMediaPage(filter: MediaFilter, cursor?: string): Promise<MediaPage> {
  try {
    const page = await internalClient.listAssets({
      ...
      ...(filter.groupBySeries ? { groupBySeries: true, groupConcurrentImages: filter.groupConcurrentImages ?? false, seriesCover: filter.seriesCover ?? 'latest' } : {}),
