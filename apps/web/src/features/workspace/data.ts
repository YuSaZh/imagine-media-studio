...
export interface MediaFilter { groupUploadedReferences?: boolean; groupConcurrentImages?: boolean; groupBySeries?: boolean; seriesCover?: 'recent' | 'latest' | 'original'; excludePrivate?: boolean; publicProjectIds?: readonly string[]; kind: 'all' | MediaKind; saved: boolean; projectId: string | null; search: string; }
export interface MediaPage { items: MediaItem[]; nextCursor: string | null; offline: boolean; }
export { ACTIVE_JOB_STATUSES, RETRYABLE_JOB_STATUSES, JOB_LABELS } from './job-status';

export function mapMedia(asset: AssetDto, job: JobDto | null = null): MediaItem {
  return {
    id: asset.id, kind: asset.type, title: job?.prompt.slice(0, 48) ?? asset.originalFilename ?? t("未命名作品"),
    ...
    model: job?.modelId ?? t("本地上传"), providerId: job?.providerId ?? 'local', saved: asset.favorite,
...
export async function fetchMediaPage(filter: MediaFilter, cursor?: string): Promise<MediaPage> {
  try {
    const page = await internalClient.listAssets({
      ...
      ...(filter.groupBySeries ? { groupBySeries: true, groupConcurrentImages: filter.groupConcurrentImages ?? false, groupUploadedReferences: filter.groupUploadedReferences ?? false, seriesCover: filter.seriesCover ?? 'latest' } : {}),
...
export async function allPages<T>(load: (cursor?: string) => Promise<{ items: readonly T[]; nextCursor: string | null }>): Promise<T[]> {
  ...
  do {
    ...
    if (cursors.has(page.nextCursor)) throw new Error(t("分页游标重复，请重新加载"));
    cursors.add(page.nextCursor);
    cursor = page.nextCursor;
  } while (cursors.size < 100);
  throw new Error(t("目录过大，请缩小查询范围"));
}
...
export function generationRequest(input: Creation): GenerationRequest {
  ...
  const { model } = input;
  if (input.count > 32 || input.count < 1 || !Number.isInteger(input.count)) throw new Error(t("生成数量应为 1 到 32"));
  const video = input.operation.startsWith('video.');
  if (!model.capabilities.operations.includes(input.operation)) throw new Error(t("当前模型不支持这项操作"));
  ...
  if (customRatio && !(customSize && parseAspectRatio(input.ratio))) throw new Error(t("当前模型不支持所选画幅"));
  ...
  if (resolution && !model.capabilities.resolutions.includes(resolution)) {
    ...
    if (!customSize || !dimensions || Number(dimensions[1]) > 16384 || Number(dimensions[2]) > 16384 || Number(dimensions[1]) * Number(dimensions[2]) > 100_000_000) throw new Error(t("分辨率须为有效像素尺寸，单边不超过 16384，总像素不超过 1 亿"));
  }
  if (video && model.capabilities.durations.length && !model.capabilities.durations.includes(input.duration)) throw new Error(t("当前模型不支持所选时长"));
  const durationRange = model.capabilities.durationRange;
  if (video && durationRange && (input.duration < durationRange.min || input.duration > durationRange.max)) throw new Error(t("视频时长超出模型范围"));
  const seed = Number(input.seed);
  if (input.seed && !Number.isSafeInteger(seed)) throw new Error(t("种子必须是整数"));
