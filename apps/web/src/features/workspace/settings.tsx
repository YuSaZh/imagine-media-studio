...
function Preferences({ online }: { online: boolean }) {
  ...
  return <div className="preferences">
    ...
    {values.groupBySeries && <label className="setting-line setting-suboption"><span>并发生图作为系列显示<small className="setting-description">将同一次提交生成的多张图片合并为一个系列。</small></span><input type="checkbox" aria-label="并发生图作为系列显示" checked={values.groupConcurrentImages} disabled={disabled} onChange={event => patch.mutate({ 'gallery.group_concurrent_images': event.target.checked })} /></label>}
    {values.groupBySeries && <label className="setting-line setting-suboption"><span>上传参考图加入系列<small className="setting-description">开启后，上传的参考图与生成结果一起显示；关闭时参考图独立显示。</small></span><input type="checkbox" aria-label="上传参考图加入系列" checked={values.groupUploadedReferences} disabled={disabled} onChange={event => patch.mutate({ 'gallery.group_uploaded_references': event.target.checked })} /></label>}
