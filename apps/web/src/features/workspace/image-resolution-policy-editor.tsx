...
export function ImageResolutionPolicyEditor({ capabilities, onChange }: { capabilities: JsonObject; onChange: (value: Record<string, unknown>) => void }) {
  ...
  const fields: Array<{ key: keyof ImageDimensionConstraints; label: string }> = [
    { key: 'multipleOf', label: t("边长对齐倍数") }, { key: 'minWidth', label: t("最小宽度") }, { key: 'minHeight', label: t("最小高度") }, { key: 'maxWidth', label: t("最大宽度") }, { key: 'maxHeight', label: t("最大高度") },
    { key: 'minPixels', label: t("最小总像素") }, { key: 'maxPixels', label: t("最大总像素") }, { key: 'maxAspectRatio', label: t("最大长短边比例") },
  ];
  return <fieldset className="parameter-editor"><legend>{t("分辨率能力")}</legend>
      <div className="form-columns">
        <label><span>{t("分辨率参数类型")}</span><Select aria-label={t("分辨率参数类型")} value={policy.mode} onChange={event => update({ ...policy, mode: event.target.value as ImageResolutionCapability['mode'] })}><SelectItem value="native">{t("原生档位")}</SelectItem><SelectItem value="pixels">{t("像素尺寸")}</SelectItem></Select></label>
        <label><span>{t("分辨率允许值（逗号分隔）")}</span><input aria-label={t("分辨率允许值")} value={valuesDraft} onChange={event => setValuesDraft(event.target.value)} onBlur={() => update({ ...policy, values: valuesDraft.split(/[,，]/).map(value => value.trim()).filter(Boolean) })} /></label>
      </div>
      <label className="check-line"><input type="checkbox" checked={policy.allowCustomDimensions} onChange={event => update({ ...policy, allowCustomDimensions: event.target.checked })} />{t("允许自定义像素尺寸")}</label>
