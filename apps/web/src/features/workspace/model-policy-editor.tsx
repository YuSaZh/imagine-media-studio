...
export function parameterPresets(capabilities: JsonObject, providerType = ''): ModelParameter[] {
  ...
  if (custom && typeof custom === 'object' && !Array.isArray(custom)) {
    ...
    if (properties && typeof properties === 'object' && !Array.isArray(properties)) for (const [key, value] of Object.entries(properties)) {
      ...
      const path = providerFamily(String(capabilities.profile ?? providerType)) === 'xai' && ['quality', 'audio'].includes(key) ? key : `extra.${key}`;
...
export function ModelPolicyEditor({ value, onChange, providerType }: { value: string; onChange: (value: string) => void; providerType: string }) {
  ...
  return <div className="model-policy-editor">
    ...
    <label className="check-line"><input type="checkbox" checked={rules !== undefined} onChange={event => { const next: Record<string, unknown> = { ...capabilities }; if (event.target.checked) next.parameters = parameterPresets(capabilities, providerType); else delete next.parameters; update(next); }} />使用自定义参数规则</label>
