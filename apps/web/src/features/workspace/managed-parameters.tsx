...
export function ManagedParameters({ rules, values, onChange, resolutionControl }: { rules: ModelParameter[]; values: JsonObject; onChange: (values: JsonObject) => void; resolutionControl?: ReactNode }) {
  ...
  return <>{rules.filter(rule => rule.enabled && rule.visible).map(rule => {
    ...
    return <label className="setting-line" key={rule.path}><span>{rule.label}{rule.required ? ' *' : ''}</span>{rule.type === 'boolean' ? <Select aria-label={rule.label} disabled={rule.locked} value={value === undefined ? '' : String(value)} onChange={event => set(rule, event.target.value === '' ? undefined : event.target.value === 'true')}><SelectItem value="">{t("默认")}</SelectItem><SelectItem value="true">{t("开启")}</SelectItem><SelectItem value="false">{t("关闭")}</SelectItem></Select> : rule.type === 'select' && !rule.allowCustom ? <Select aria-label={rule.label} disabled={rule.locked} value={value === undefined ? '' : String(value)} onChange={event => set(rule, options?.find(option => String(option) === event.target.value))}><SelectItem value="">{t("默认")}</SelectItem>{options?.map(option => <SelectItem key={String(option)} value={String(option)}>{String(option)}</SelectItem>)}</Select> : <input aria-label={rule.label} disabled={rule.locked} type={rule.type === 'number' ? 'number' : 'text'} min={rule.min} max={rule.max} step={rule.step ?? 'any'} value={typeof value === 'number' || typeof value === 'string' ? value : ''} placeholder={t("默认")} onChange={event => set(rule, event.target.value === '' ? undefined : rule.type === 'number' ? Number(event.target.value) : event.target.value)} />}</label>;
  })}</>;
}
