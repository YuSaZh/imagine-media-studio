...
export function ImageResolutionPicker({ model, rules, value, ratio, onChange, onUnlock, label = t("分辨率"), trigger, className = '' }: ResolutionProps & { label?: string; trigger?: ReactNode; className?: string }) {
  ...
  return <Popover.Root open={open} onOpenChange={open => { setOpen(open); setCustom(false); setPendingPreset(''); }}>
    ...
    <Popover.Portal><Popover.Content className="options image-resolution-options" aria-label={t("图片分辨率")} sideOffset={10} collisionPadding={12}>
      <div className="option-heading">{pendingPreset ? t("{0} · 画幅", [pendingPreset]) : t("图片分辨率")}</div>
      ...
      <button type="button" className={`choice ${custom || selected === t("自定义") ? 'is-active' : ''}`} aria-pressed={custom || selected === t("自定义")} disabled={!customAllowed} onClick={() => setCustom(true)}>{t("自定义")}</button>
...
export function ImageResolutionSetting(props: ResolutionProps) {
  ...
  return <div className="setting-line"><span>{rule?.label ?? t("分辨率")}</span><ImageResolutionPicker {...props} label={rule?.label ?? t("分辨率")} trigger={<SettingValue>{imageResolutionLabel(props.value)}</SettingValue>} /></div>;
}
