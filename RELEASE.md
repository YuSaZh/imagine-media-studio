# Imagine Media Studio v0.1.3 Release Guide

Imagine Media Studio `v0.1.3` improves desktop/mobile generation controls,
independent task counts, image protocol compatibility, and large-image handling.
...
the [GitHub Release](https://github.com/YuSaZh/imagine-media-studio/releases/tag/v0.1.3)
...
For `v0.1.3`, take the exact digest from the GitHub Release or release workflow
summary:
...
docker run --rm \
  ...
  --target /recovery/restored-v0.1.3
```

Inspect the restored tree, recreate the application container with
`imagine-state/restored-v0.1.3` bound to `/data`, and keep the same
...
Use the digest, not `0.1.3`, `0.1`, or `latest`, as the verification subject:

Run these commands from a verified `v0.1.3` source checkout. GitHub CLI must be
